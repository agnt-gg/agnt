...
## Partial-work continuation and failure evidence — 2026-09-11
The previous two-choice resolution contract could not represent partially executed work. It now also accepts outcome continue_partial with exact taskId/attemptId, evidence, workerStopped:true, effectsReconciled:true, bounded remainingWork, nonempty doNotRepeat instructions and reviewed artifacts [{path,sha256}]. Hashes and evidence are operator assertions; this API does not read arbitrary paths or independently verify external effects. Do not assert effectsReconciled while any effect remains unknown.

Resolution preserves prior task output/error/status in the resolution receipt and the old attempt in history as resolved_continue_partial. The task becomes pending with outcome:partial and a continuation handoff, not completed. Resolution does not start execution or override Pause. Local and upgraded remote workers receive the handoff and must check artifact hashes before editing. Invalid/missing-task/old-run/wrong-attempt and repeated decisions refuse atomically.

Owned task failures append a diagnostic bound to the exact attempt before rethrowing. Diagnostics survive lease expiry without changing tasks or granting retry. The additive goal_run_failures table stores one diagnostic per attempt: bounded message/code/name and available tool-name/status metadata. Raw stacks, tool arguments/results and arbitrary objects are excluded; known token forms are redacted. Arbitrary free-text secrets cannot be guaranteed detectable. Missing receipts stay missing, and old lost exceptions cannot be reconstructed. Diagnostic-write failure preserves the original error and barrier.

Fresh verification: 6056 backend passed, one skipped; 38 frontend guard/theme/panel/card tests passed; production build passed. RED recorded five failures/50 passes (initial wrapper timeout retained), then all passed; UI partial acceptance failed before wiring. Missing-method tests are contract failures, not preexisting behavioral reproductions. Coverage includes exact attempt, preserved history/output, pause, duplicate/refused decisions, expired diagnostics, redaction, runner catch and continuation instructions.

This update is not deployed. PR2's previous exception remains unknown and its admitted attempt was not falsely resolved. New partial continuation requires verified effects and worker termination before admission.
