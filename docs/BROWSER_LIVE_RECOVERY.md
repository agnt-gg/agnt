...
- Real OS suspension and two-account UI switching are disclosed untested scenarios, not established repository-required checks or blockers to requesting maintainer review. They are not claimed passed; maintainers may require them before release.
...
- This patch and its subsequent hardening have been installed and tested locally. Review readiness is not maintainer approval or merge authorization.
...
- **Cold Chromium startup:** two CI attempts failed the first browserActDriver live handshake at 4s. The three-file browser fix from PR #131 commit `84d5f643` is now included unchanged: configurable bounded handshake (default 4s, validated 1–30000ms), driver-specific 15s startup budget, and eight startup contract tests. RED: all eight failed without the fix; GREEN: eight startup + nine actual Chromium cases pass. No action retry. This deliberately allows slower cold connections, not a speed improvement. Merge coordination with #131 is required because the same three files now overlap; no goal changes were imported.
...
- **Local deployment evidence (2026-09-10):** hardening at `53988122` and companion toolbar `9e347ff0` was installed with backups and a matching frontend; supervised restart changed PID 828927 to 2404985. Post-deployment acceptance against the real backend/authentication passed canvas paint, click/page feedback, two viewers, close-one, reconnect, auth rejection/reauthentication and cleanup. Non-viewing sockets received no image events. Evidence: `browser-hardening-deploy-ckWBx5/completed-verification.json` and `browser-hardening-acceptance-x4zvtv/live-results-v4.json` in the operator workspace. The host was a temporary component window, not inspection of the user's Electron window; OS suspension and two real accounts remain untested. These are historical deployment observations, not proof that a later changed checkout is identical.
