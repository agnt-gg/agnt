...
| `icon` | ✅ | Icon key. |
| `entry` | ✅ | JS file implementing the tool. |
| `parameters` | ✅ | Object of input definitions (see §3). |
| `outputs` | ✅ | Output schema (see §5). |
| `authRequired` / `authProvider` | optional | See §6. |
...
The entry file default-exports a class with an `execute` method. The signature
is **always**:
...
```bash
node build-plugin.js <path-to-your-plugin-folder>   # → <name>.agnt
```

The `.agnt` is a portable, gzipped bundle with dependencies pre-installed:
install it via **Settings → Plugins → Install from file**, or publish it to the
**marketplace**. Dependencies are installed automatically at install/load time —
the end user needs no npm/Node toolchain.
