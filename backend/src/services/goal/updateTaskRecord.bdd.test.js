...
describe('persisted task updates, never simulated success',()=>{
 it('Given committed write and readback failure, Then declare committed uncertainty and prohibit retry',async()=>{Task.findOne.mockResolvedValueOnce({id:'t',goal_id:'g',status:'completed'}).mockRejectedValueOnce(Error('read unavailable'));await expect(updateTaskRecord({goal_id:'g',task_id:'t',status:'failed'},'u')).rejects.toMatchObject({code:'TASK_UPDATE_UNCERTAIN',writeState:'committed',retryable:false});expect(Task.updateStatus).toHaveBeenCalledOnce();});
 it('Given storage error during write, Then do not claim no effects',async()=>{Task.updateStatus.mockRejectedValueOnce(Error('write interrupted'));await expect(updateTaskRecord({goal_id:'g',task_id:'t',status:'failed'},'u')).rejects.toMatchObject({code:'TASK_UPDATE_UNCERTAIN',writeState:'unknown',retryable:false});});
