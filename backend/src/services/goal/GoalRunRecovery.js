...
export default {
  ...
  async waitRemote(lease,signal,localNodeId) {
    const s=await store();
    while(true){
      ...
      const work=await s.remoteWork(lease,localNodeId);
      if(work==='none')return;
      if(work==='unknown')throw Error('Remote task outcome unknown; reconciliation required');
