...
class GeminiCliAuthManager {
  ...
  async checkApiUsable({ forceRefresh = false } = {}) {
    ...
    try {
      ...
      const status = error.response?.status;
      // Google discontinued Gemini CLI consumer OAuth (free / AI Pro / AI Ultra)
      // on June 18, 2026 (PRD-107). A 403 / PERMISSION_DENIED on the OAuth health
      // check WITHOUT an enterprise GCP project set means the consumer path is
      // dead. Enterprise (Code Assist Standard/Enterprise) still works, so only
      // flag deprecation when there's no GCP project configured.
      const errStatus = error.response?.data?.error?.status;
      const isConsumerOAuth = !this.isUsingApiKey() && !this._readGcpProject();
      const deprecated = isConsumerOAuth && (status === 403 || errStatus === 'PERMISSION_DENIED');
      const result = {
        available: true,
        apiUsable: false,
        ...
        hint: deprecated
          ...
          : status === 401 ? 'Token expired or revoked' : `API error: ${error.message}`,
      };
      this._lastApiStatus = result;
      this._lastApiCheck = Date.now();
      return result;
