...
describe('opt-in native Codex image integration',()=>{
 ...
 it('chat refuses an unsupported policy before adapter dispatch',async()=>{state.generate.mockClear();const out=JSON.parse(await TOOLS.generate_image.execute({prompt:'x',provider:'openai-codex',model:'unsupported-policy'},null,{userId:'fixture'}));expect(out.success).toBe(false);expect(state.generate).not.toHaveBeenCalled();});
});
