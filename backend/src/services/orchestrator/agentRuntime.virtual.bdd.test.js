...
describe('virtual goal worker runtime',()=>{
 it('Given no saved worker row, Then retain its explicit core and shell tools',async()=>{
  ...
  for(const name of worker.assignedTools)expect(namesOf(runtime.toolSchemas).has(name)).toBe(true);
  expect(runtime.context._toolCeiling.has('send_email')).toBe(false);
 });
});

it('Given a missing declared virtual tool, Then fail before model execution instead of narrating',async()=>{AgentModel.findOne.mockResolvedValue(null);await expect(buildAgentRuntime({agentId:'built-in-task-executor',userId:USER_ID,builtInAgent:{id:'built-in-task-executor',isBuiltIn:true,assignedTools:['missing-tool']}})).rejects.toThrow(/tools unavailable/);});
