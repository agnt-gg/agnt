/**
 * Computer use — the shared shim over the Cua Driver (trycua/cua, MIT).
 *
 ...
 *
 * AGNT_CUA_DRIVER_PATH overrides binary resolution (tests point it at a fake).
 */
import fs from 'fs';
...
export function resolveDriverPath() {
  const override = String(process.env.AGNT_CUA_DRIVER_PATH || '').trim();
  if (override) return { found: fs.existsSync(override), path: override, onPath: false };
...
/**
 * How to spawn the driver. A JS file as the override (tests) runs under this
 * node: Node 22 refuses .cmd/.bat shims without a shell (EINVAL), and a shell
 * is the one thing a JSON argument must never pass through.
 */
function spawnSpec(args) {
  const { path: bin } = resolveDriverPath();
  if (/\.(c|m)?js$/i.test(bin)) return { command: process.execPath, args: [bin, ...args] };
...
export function startDaemon() {
  return new Promise((resolve) => {
    let child;
    try {
      const spec = spawnSpec(['serve']);
      child = spawn(spec.command, spec.args, { detached: true, stdio: 'ignore', windowsHide: true });
      child.unref();
    } catch (e) {
