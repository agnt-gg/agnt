...
description: "Generate an AI image, quantize it to a 256-color indexed palette, and produce a self-contained HTML artifact and a perfect-loop animated GIF using classic 1990s palette cycling. Rotating palette entries makes water flow, fire flicker, and stars twinkle without redrawing pixels. Optionally freeze a subject or UI elements so only the background animates, using pixel-perfect masks derived directly from the indexed image. Use when the user asks to color-cycle an image, make a generated image animate, create old-school palette animation, Mark Ferrari style effects, 8-bit/16-bit animated scenes, or indexed-color animation. Also triggers on canvascycle, palette rotation, flowing water, glowing rivers, or keeping a character still while the background moves."
version: 1.0.0
---
