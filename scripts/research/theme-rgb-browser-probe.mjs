...
import { collectPalette } from './theme-rgb-palette.mjs';
...
const sources = [mainFile, ...files, rendererFile, 'scripts/research/theme-rgb-browser-probe.mjs', 'scripts/research/theme-rgb-palette.mjs'].map((file) => ({
  file,
...
try {
  ...
  const palette = await page.evaluate(collectPalette, { themes, mapping });
  ...
  console.log(JSON.stringify({
    schema: 'agnt.theme-rgb-browser-research.v2',
    ...
    summary: {
      ...
      differentPixels: palette.comparisons.filter((comparison) => comparison.equalPixels === false).length,
      inconclusive: palette.comparisons.filter((comparison) => comparison.equalPixels === null).length,
    },
    palette,
    capture,
  }, null, 2));
