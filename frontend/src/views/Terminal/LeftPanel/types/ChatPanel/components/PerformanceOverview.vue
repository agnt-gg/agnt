...
export default {
  ...
  setup() {
    ...
    const automationEfficiency = computed(() => store.getters['userStats/roiPercentage'] || 0);
    const tasksProcessed = computed(() => store.getters['userStats/totalWorkflows'] || 0);
    ...
    const animatePerformanceChart = () => {
      ...
      ctx.lineWidth = 4;
      ctx.beginPath();
      ctx.arc(centerX, centerY, radius, -Math.PI / 2, -Math.PI / 2 + Math.PI * 2 * efficiencyValue, false);
      ctx.stroke();
    };
